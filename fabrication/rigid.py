...
class rigid_info:
    def __init__(self, mesh_name, fix_mass, mesh_mass_center_global, seed_mass_center,
                 seed_mass_ratio, mesh_scale, mesh_translation, mesh_init_amom, mesh_init_vel, gravity, mesh_euler,
                 addweight_beginpoint, addweight_endpoint, addweight_mass_rho, mass_coeff, leaf_rho, c_L):
        ...
        # addweight
        self.addweight_beginpoint = addweight_beginpoint
        self.addweight_endpoint = addweight_endpoint
        self.addweight_mass_rho = addweight_mass_rho

        # leafweight
        self.mass_coeff = mass_coeff
        self.leaf_rho = leaf_rho
        self.c_L = c_L
...
class rigid_body:
    def __init__(self, _nx, _ny, _nz, info):
        ...
        self.mesh.verts.place({'pos': vec3, 'radius': vec3, "areas": vec3, 'dual_area': float_type})
        ...
        # addweight
        self.additional_weight_beginpoint = ti.Vector.field(3, shape=(), dtype=ti.f32)
        self.additional_weight_endpoint = ti.Vector.field(3, shape=(), dtype=ti.f32)
        ...
        self.total_area = ti.field(float_type, shape=())
        self.leaf_center = ti.Vector.field(3, shape=(), dtype=float_type)
        self.mass_coeff = 0
    ...
    def reinitialize_simulation(self, info, pos_torch):
        ...
        self.mass_coeff = info.mass_coeff

        self.compute_areas()
        self.total_area[None] = self.total_area[None] * info.mesh_scale[0] ** 2
        # self.leaf_center[None] = self.leaf_center[None]

        # addweight
        self.additional_weight_beginpoint[None] = info.addweight_beginpoint
        self.additional_weight_endpoint[None] = info.addweight_endpoint
        self.addweight_mass_rho = info.addweight_mass_rho
        self.addweight_mass = np.linalg.norm(self.additional_weight_beginpoint[None].to_numpy() - self.additional_weight_endpoint[None].to_numpy()) \
                              * self.addweight_mass_rho * info.c_L * self.mass_coeff * info.mesh_scale[0]

        self.global_mass_center[None] = info.mesh_mass_center_global
        self.grav_accr = vec3(0, 0, info.gravity)
        self.fix_mass = info.fix_mass
        self.leaf_mass = self.total_area[None] * info.leaf_rho * info.c_L ** 2 * self.mass_coeff
        self.mass = self.fix_mass + self.leaf_mass + self.addweight_mass
        ...
        self.force[None] = self.torque[None] = self.ang_vel[None] = vec3(0)
        self.compute_radius()
    ...
    def compute_mass_center(self):
        m_seed = self.fix_mass
        m_addweight = self.addweight_mass
        # m_addweight = self.additional_weight_mass_ratio * self.mass
        m_rest = self.leaf_mass

        c_rest = vec3(0.0)
        for v in self.mesh.verts:
            c_rest += v.pos
        c_rest = self.leaf_center[None]
        # print("crest=", c_rest / self.nverts)
        # print("self.leaf_center[None]=", self.leaf_center[None])
        # self.mass_center[None] = m_rest * c_rest / self.nverts
        self.mass_center[None] = m_rest * c_rest

        self.mass_center[None] += m_seed * self.seed_mass_center[None]

        self.mass_center[None] += m_addweight * (self.additional_weight_beginpoint[None] + self.additional_weight_endpoint[None]) / 2.0

        self.mass_center[None] /= self.mass
        # print("first self.mass_center[None]=", self.mass_center[None])
    ...
    def compute_inertia_tensor(self):
        ...
        #     I += I_v
        m_seed = self.fix_mass
        # m_addwei = self.additional_weight_mass_ratio * self.mass / 100.0 ??????
        m_addwei = self.addweight_mass / 100.0
        m_rest = self.leaf_mass
        # m_v = (1.0 - self.seed_mass_ratio - self.additional_weight_mass_ratio) * self.mass / self.nverts
        m_v = m_rest / self.nverts
        m_v_area = m_rest / self.total_area[None]

        I = mat3(0)

        for v in self.mesh.verts:
            I_v = m_v_area * v.areas[0] * (v.radius.norm_sqr() * ti.Matrix.identity(ti.f32, 3) - v.radius.outer_product(v.radius))
            I += I_v
        # print(I)  
        ...
        for i in range(100):
            pos_addwei = self.additional_weight_beginpoint[None] * (99.0 - i) / 99.0 + self.additional_weight_endpoint[None] * i / 99.0
            r_addwei = pos_addwei - self.mass_center[None]
            I_addwei = m_addwei * (r_addwei.norm_sqr() * ti.Matrix.identity(ti.f32, 3) - r_addwei.outer_product(r_addwei))
            I += I_addwei

        self.inertia_tensor[None] = I
        self.inertia_tensor_inv[None] = I.inverse()
        # print(self.inertia_tensor[None])
    ...
    def transform(self, scale: vec3, translation: vec3, euler: vec3):
        ...
        # print("self.mass_center[None]=", self.mass_center[None])
        ...
        # print("self.seed_mass_center[None]=", self.seed_mass_center[None])
        self.additional_weight_beginpoint[None] = R @ (self.additional_weight_beginpoint[None] * scale) + trans
        self.additional_weight_endpoint[None] = R @ (self.additional_weight_endpoint[None] * scale) + trans
        # print("self.additional_weight_beginpoint[None]=", self.additional_weight_beginpoint[None])
        # print("self.additional_weight_endpoint[None]=", self.additional_weight_endpoint[None])
        ...
        self.leaf_center[None] = R @ (self.leaf_center[None] * scale) + trans
        for v in self.mesh.verts:
            # v.pos = R @ (v.pos * scale) + translation
            v.pos = R @ (v.pos * scale) + trans
            v.areas = v.areas * scale ** 2

    @ti.kernel
    def compute_areas(self):
        tmp = vec3(0)
        self.total_area[None] = 0
        # v0_id = next(self.mesh.faces).verts[0]
        # print(v0_id)
        for v in self.mesh.verts:
            v.areas[0] = 0.0
        for f in self.mesh.faces:
            ...
            tmp += f.area * (v1 + v2 + v0) / 3
            # v0_id = f.verts[0]
            # print(v0_id)
            f.verts[0].areas[0] += f.area / 3
            f.verts[1].areas[0] += f.area / 3
            f.verts[2].areas[0] += f.area / 3
        self.leaf_center[None] = tmp / self.total_area[None]
        # jj = 0.
        # for v in self.mesh.verts:
        #     jj += v.dual_area
        # print(jj)
