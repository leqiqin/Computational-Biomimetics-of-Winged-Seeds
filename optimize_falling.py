...
input_tensor = torch.tensor([[1., 0., 0., 0., 0., 0., 0., 0.]]).to(torchdeviceId).requires_grad_(True)
...
for step in range(100):
    loss = compute_loss_falling(input_tensor[0], sigma_1, interp)[0]
    ...
    while len(f_outputs) < n:
        ...
        f_output, prob = compute_loss_falling(noisy_input[0], sigma_1, interp)
        if f_output < 999998:
