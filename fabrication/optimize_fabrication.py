...
input_tensor = torch.tensor([[0., 0., 0., 0., 1.]]).to(torchdeviceId).requires_grad_(True)
...
for step in range(100):
    loss = compute_loss_rot(input_tensor[0], sigma_1, interpolate)[0]
    ...
    while len(f_outputs) < n:
        ...
        f_output, prob = compute_loss_rot(noisy_input[0], sigma_1, interpolate)
        if f_output < 999998:
